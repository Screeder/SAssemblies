...
namespace SAssemblies.Trackers
{
    class Uim
    {
        ...
        private static SpriteHelperNew.SpriteInfo championSprite = new SpriteHelperNew.SpriteInfo();
        ...
        void Game_OnGameUpdate(EventArgs args)
        {
            ...
            foreach (var enemy in _enemies)
            {
                ...
                if (championSprite.Bitmap.Count == 0 || championSprite.Bitmap.Find(x => x.Name.Contains(enemy.Key.ChampionName)) == null)
                {
                    SpriteHelperNew.LoadTexture(enemy.Key.BaseSkinName, ref championSprite, "UIM");
                    Game.PrintChat("Downloading {0}", enemy.Key.ChampionName);
                }
                if (championSprite.Bitmap.Find(x => x.Name.Contains(enemy.Key.ChampionName) && x.DownloadFinished && !x.LoadingFinished) != null)
                {
                    Game.PrintChat("Loading {0}", enemy.Key.BaseSkinName);
                    SpriteHelperNew.SpriteInfo.BitmapInfo bInfo = championSprite.Bitmap.Find(x => x.Name.Contains(enemy.Key.ChampionName));
                    Render.LSBitmap bitmap = bInfo.Bitmap;
                    bitmap.GrayScale();
                    bitmap.TextureBitmap = CropImage(bitmap.TextureBitmap, bitmap.Width);
                    bitmap.Scale = new Vector2(((float)24 / bitmap.Width) * percentScale, ((float)24 / bitmap.Height) * percentScale);
                    bitmap.PositionUpdate = delegate
                    {
                    ...
                    bitmap.VisibleCondition = delegate
                    {
                        return Tracker.Trackers.GetActive() && UimTracker.GetActive() && !enemy.Key.IsVisible;
                    };
                    bInfo.LoadingFinished = true;
                }

                //if (enemy.Value.SpriteInfo == null || !enemy.Value.SpriteInfo.Bitmap.All(x => x.Name.Contains(enemy.Key.ChampionName)))
                //{
                //    SpriteHelperNew.LoadTexture(enemy.Key.ChampionName, ref enemy.Value.SpriteInfo, "UIM");
                //    //SpriteHelper.LoadTexture(enemy.Key.ChampionName, ref enemy.Value.SpriteInfo, null, RafLoader.ImageList.ChampionCircle);
                //}
                //if (enemy.Value.SpriteInfo != null && enemy.Value.SpriteInfo.Bitmap.All(x => x.DownloadFinished) && !enemy.Value.SpriteInfo.LoadingFinished)
                //{
            ...
            if (championSprite.Bitmap.Count == _enemies.Count && championSprite.Bitmap.All(x => x.LoadingFinished) && !championSprite.LoadingFinished)
            {
                SpriteHelperNew.PrepareSprite(ref championSprite);
                championSprite.Sprite.Add(0);
                championSprite.LoadingFinished = true;
            }
        }
        ...
        class InternalUimTracker
        {
            ...
            public SpriteHelperNew.SpriteInfo SpriteInfo;
